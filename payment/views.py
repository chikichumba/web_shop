...
def create_stripe_chekout_session(order, request):
    ...
    for item in cart.items.select_related('product', 'product_size'):
        line_items.append({
            'price_data':{
                'currency':'eur',
                'product_data':{
                    'name': f"{item.product.name} - {item.product.size.name}",
                },
                'unit_ammount': int(item.product.size*100)
            },
        ...
        try:
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='payment',
                success_url=requests.build_absolute_uri('/payment/stripe/success') + '/session_id={CHECKOUT_SESSION_ID}',
